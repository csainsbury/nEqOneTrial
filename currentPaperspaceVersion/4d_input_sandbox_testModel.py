...
a = Input(shape = (numberOfIDs, 30))
b = Input(shape = (numberOfIDs, 30))
c = Input(shape = (numberOfIDs, 30))
d = Input(shape = (numberOfIDs, 30))

# em = Embedding(numberOfCombinations, embedding_vector_length, input_length = numberOfTimesteps)(a)
em = Embedding(200, output_dim = 256)(a)

b2 = Reshape((numberOfIDs, 30, 1))(b)
c2 = Reshape((numberOfIDs, 30, 1))(c)
d2 = Reshape((numberOfIDs, 30, 1))(d)

merged = merge([em, b2, c2, d2], mode = 'concat')

rnn = LSTM(128)(merged.shape[2:])

M = Model(input=[a, b, c, d], output=[rnn])
...
M.evaluate([X_train_set4, X_train_set1, X_train_set2, X_train_set3], y_train)
...
rnn.add(LSTM(return_sequences=True, input_shape = , units=128))
rnn.add(Dropout(0.5))
