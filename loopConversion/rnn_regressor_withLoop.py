...
for loopn in range(1, numberOfLoops):
    ...
    model.compile(optimizer='adam', loss='mean_squared_error', loss_weights=[0.2, 0.4])
    ...
    y_testTransform = yScaler.inverse_transform(y_test)
    y_pred_asNumberTransform = yScaler.inverse_transform(y_pred_asNumber)

    ##
    ax = plt.gca()
    ax.scatter(y_testTransform, y_pred_asNumberTransform[0],s = 0.1, label = 'pred vs y')
    #ax.set_xscale('log')
    #ax.set_yscale('log')
    ax.legend()
    ...
    #np.savetxt('./pythonOutput/y_preTest.csv', hba1cScaler.inverse_transform(X_test_numericalTS[:, 47, 0]), fmt='%.18e', delimiter=',')
    np.savetxt('./pythonOutput/y_test.csv', yScaler.inverse_transform(y_test), fmt='%.18e', delimiter=',')
    hba1cVals = hba1cScaler.inverse_transform(X_test_numericalTS[:, :, 0])
    preTestVals = hba1cVals[:, 47]
    ...
    for r_count in range(0, len(therapyArray), 1):
        ...
        y_pred_asNumber_substituteTransform = yScaler.inverse_transform(y_pred_asNumber_substitute[0])
        np.savetxt('./pythonOutput/y_pred_asNumber_combinationNumber_' + str(r_count) + '_' + str(therapyArray[r_count]) + '.csv', y_pred_asNumber_substitute[0], fmt='%.18e', delimiter=',')
        np.savetxt('./pythonOutput/y_pred_asNumber_combinationNumberTransform_' + str(r_count) + '_' + str(therapyArray[r_count]) + '.csv', y_pred_asNumber_substituteTransform, fmt='%.18e', delimiter=',')
        np.savetxt('./pythonOutput/y_pred_asNumber_combinationNumberTransform_' + str(r_count) + '_runN_%s.csv' % loopn, y_pred_asNumber_substituteTransform, fmt='%.18e', delimiter=',')

        ay = plt.gca()
        ay.scatter(preTestVals, y_pred_asNumber_substituteTransform,s = 0.1, label = 'pred vs y')
        #ax.set_xscale('log')
        #ax.set_yscale('log')
        #ay.legend()
        plt.savefig('./pythonOutput/plot_combinationNumber_' + str(r_count) + '.png', dpi = 600)
        plt.clf()
    ...
    np.savetxt('./pythonOutput/X_test_age.csv', ageScaler.inverse_transform(X_test_age), fmt='%.18e', delimiter=',')
